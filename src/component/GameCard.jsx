...
export default function GameCard({ img, title, provider, platform, gameCode, gameType, game }) {
  ...
  const getGameURL = () => {
    //setLoading(true);
    // console.log("🚀 ~ getGameURL ~ data:", data)
    ...
    const ip = '';
    console.log(">>>>>>>>>>>>>>>>>>>", ip)
    // const gameUrl = `https://${ip}/?token=${authToken}&game_code=${gameCode}&opr_id=${oprId}&href=${window.location.href}`;
    // const gameUrl = `http://192.168.0.59:5175/?token=${authToken}&game_code=${gameCode}&opr_id=${oprId}&href=${window.location.href}`;
    const gameUrl = `https://3rdp-games.iomhost.com/?token=${authToken}&server_ip=${server_ip}&game_name=${title}&server_port=${server_port}&game_code=${gameCode}&opr_id=${import.meta.env.VITE_APP_OPERATOR_ID}&player_id=${userId}&provider_code=${platform}&room_code=&href=${window.location.href}`;
    ...
    <>
      ...
      >
        <div className="nineWickets-inner-main">
          ...
          <p className="trunicate-text">{title}</p>
        </div>

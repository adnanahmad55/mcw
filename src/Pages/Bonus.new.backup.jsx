...
export default function Bonus() {
    ...
    const [claimData, setClaimData] = useState(""); // Can now hold {type: 'turnover', id: '...'} or string ID
    ...
    // Function to fetch turnover data
    const fetchTurnoverData = async () => {
        try {
            ...
            if (res?.data?.status) {
                 // Filter to get only claimable turnover items
                 const claimableTurnover = res?.data?.data?.turnover?.promotions?.filter(item => item?.is_claimable && !item?.is_claimed) || [];
                 setTurnoverData(claimableTurnover);
            } else {
                console.warn("No turnover data found or API error:", res?.data?.message);
            }
        } catch (err) {
            console.error("Get turnover data error:", err);
        }
    };

    // Function to claim turnover bonus
    // Function to claim turnover bonus
    const claimTurnoverBonus = async (turnoverId) => {
        // Find the bonus amount *before* claiming
        const bonusItem = turnoverData.find(item => item._id === turnoverId);
        const bonusAmount = bonusItem ? bonusItem.amount : 0;
        ...
        try {
            ...
            if (res?.data?.status) {
                ...
                if (res?.data?.data?.redirect_url) {
                    window.open(res.data.data.redirect_url, '_blank'); // Open in new tab
                }

                // Use the amount found before the claim (or fallback to response if needed)
                // If the redirect happens, you might still want to show the congrats screen locally
                // or handle it differently based on business logic.
                // For now, we'll assume showing the congrats screen locally is still desired.
                setClaimAmount(bonusAmount || res?.data?.data?.amount || 0); // Fallback to response amount if lookup failed
                setClaimAmountModal(true); // Show congratulations screen
                // Refresh data after claiming
                fetchTurnoverData();
                // Optionally, update wallet balance if API provides it
                 if (res?.data?.data?.wallet_balance !== undefined) {
                     localStorage.setItem('wallet_balance', res.data.data.wallet_balance);
                 }
            } else {
                alert(res?.data?.message || "Failed to claim turnover bonus");
    ...
    const claimInstantWinBonus = async (claimId) => {
        try {
            ...
            if (response?.data?.status) {
                ...
                fetchVipData();
                checkDailyBonus();
                fetchTurnoverData(); // Also refresh turnover data
            } else {
    ...
    const claimVipBonus = async () => {
        try {
            ...
            if (res?.data?.status) {
                // DO NOT call onClose() here yet, let the counter/timer flow handle it
    ...
    useEffect(() => {
        ...
        checkDailyBonus();
        fetchInstantWinBonus();
        fetchTurnoverData(); // Fetch turnover data on component load
    }, []);
    ...
    const getBonusType = () => {
        ...
        if (typeof claimData === 'object' && claimData.type === 'turnover') return 'turnover'; // Check for turnover object
        return 'vip-bonus';
    };
    ...
    return (
        <>
                    ...
                    <div key={idx} className='w-full h-[120px] mb-4 flex items-center px-2'
                        ...
                        <div className='px-4 gap-3 relative flex'
                            ...
                            <div className='rounded-lg py-2 px-1 w-[104px] flex items-center'
                                style={{ background: 'linear-gradient(0deg,#ff73c3,#f425d4)', boxShadow: '0 2px 9px rgba(254,52,216,.35)' }}>
                                <div className='w-full text-center text-[10px] text-white'>
                                    <span className='block text-center'>VIP REWARD</span>
                                    <span className='block text-center'>Red Package</span>
                    ...
                    <div key={item._id} className='w-full h-[120px] mb-4 flex items-center px-2'
                        ...
                        <div className='px-4 gap-3 relative flex'
                            ...
                            <div className='rounded-lg py-2 px-1 w-[104px] flex items-center'
                                style={{ background: 'linear-gradient(0deg,#ff73c3,#f425d4)', boxShadow: '0 2px 9px rgba(254,52,216,.35)' }}>
                                <div className='w-full text-center text-[10px] text-white'>
                                    <span className='block text-center'>TURNOVER</span>
                                    <span className='block text-center'>Red Package</span>
                                    {/* Use title or amount or other unique identifier */}
                                    <span className='block text-center'>{item?.title || `Turnover ${idx + 1}`}</span>
                                </div>
                            </div>
                            <div className='flex items-center'
                            >
                                <div className='w-full text-center text-[10px] text-black'>
                                    <span className='block text-left'>Reward</span>
                                    <span className='block text-left'>Amount: ৳ {item?.amount}</span>
                                    <span className='block text-left'>Type: {item.wallet_type === "cash" ? "Cash" : "Bonus"}</span>
                        ...
                        <div className='bg-white h-full text-[#bababa] text-[11px] py-[8px] px-[3px] w-[84px]'>
                            ...
                            <div className='text-[9px] font-normal text-center text-[#666]'>
                                <strong className='font-bold text-[32px]'>100%</strong> {/* Assuming completed if claimable */}
                                <p className='text-[11px] font-normal'>Complete</p>
                                {/* Show Claim button only if claimable */}
                                {item?.is_claimable && !item?.is_claimed && (
                                    <button className='w-full text-white bg-[#30d005] rounded-3xl font-normal text-center py-[6px] px-1'
                                        onClick={() => {
                                            // For turnover, set claimData as an object and open modal
                                            setClaimModal(true);
                                            setClaimData({ type: 'turnover', id: item._id }); // Pass type and ID
                                        }}>Claim</button>
                                )}
                                {/* Show Claimed status if already claimed */}
                                {item?.is_claimed && (
                                    <div className='w-full text-white bg-[#777] rounded-3xl font-normal text-center py-[6px] px-1'>
                                        Claimed
                                    </div>
                                )}
                            </div>
            ...
            {/* VIP, Instant Win, and Turnover Bonus Modal - Unified (without counter for instant/turnover) */}
            {claimModal && bonusType !== 'daily-bonus' && (
                <div className='bg-[#000000cc] fixed top-0 bottom-0 left-0 right-0 z-30'>
                    <div className='w-full p-2 relative'>
                        <IoCloseCircleOutline className='text-white absolute right-4 text-2xl top-4' onClick={onClose} />
                        ...
                        <div className='flex justify-center'>
                            <div className='w-[66px] bg-[#787878] rounded-md text-white mt-2 p-2 text-center py-3 text-[9px]'>
                                {bonusType === 'instant-win' ? 'INSTANT WIN' : bonusType === 'turnover' ? 'TURNOVER' : 'VIP BONUS'}
                            </div>
                        </div>
                        <div className='w-full text-center text-[#ffb30b] text-[13px] font-bold mt-1'>
                            {bonusType === 'instant-win' ? 'INSTANT WIN BONUS' : bonusType === 'turnover' ? 'TURNOVER BONUS' : 'VIP BONUS'}
                        </div>
                        <div className='w-full flex justify-center px-4'>
                            <img src={redenvelope} className='w-full' />
                        </div>
                        <div className='flex justify-center'>
                            <button className='w-[168px] text-center text-[#d63000] h-[26px] text-[13px] rounded-3xl'
                                ...
                                onClick={() => {
                                    // Claim directly without counter for instant bonuses and turnover
                                    if (bonusType === 'instant-win') {
                                        claimInstantWinBonus(claimData); // Call claim function directly
                                    } else if (bonusType === 'turnover') {
                                        claimTurnoverBonus(claimData.id); // Call claim function directly
                                    } else {
                            ...
                            <div className='w-full h-full absolute top-0 bottom-0 right-0 left-0 z-20'
                                onClick={() => {
                                    if (claimData === 'daily-bonus') {
                                        ...
                                        claimVipBonus(); // This starts the VIP claim process, onClose() happens later
                                    }
                                }}>
